...
| P2 | **Migration 019 applied** to the Ask customer DB via the narrow path `scripts/claim-v2-019-apply.ts` (verify → rehearse → apply --confirm → verify; see R4 doc §3) | `SELECT count(*) FROM information_schema.columns WHERE table_name='ath_claims' AND column_name IN ('acquisition_source','review_started_at','needs_info_entered_at');` → `3`, and `SELECT to_regclass('public.ath_claim_review_sessions');` not null |
