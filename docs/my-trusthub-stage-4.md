...
Follow-up revalidation at 2026-09-10 14:06:56 UTC confirmed the same production mains/deployments and daily cron. An HTTP HEAD request confirmed the extract still has Last-Modified 10:48:32 UTC and size 46,172,026 bytes; the unchanged file was not downloaded again. DBPR's [construction renewal FAQ](https://www2.myfloridalicense.com/construction-industry/faqs/) explains that Delinquent,Active means the license was not renewed by its expiration date and Active describes its prior status. The specialist's bulk-only secondary-status mapping must therefore not be reused as a portal fallback that would normalize this result to active. No compatible alternative input was established. The portal's rendered page clock is not certified as the underlying record's publication time. This follow-up changes neither the selected coverage nor the source-health result.
