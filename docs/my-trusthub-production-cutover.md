...
The verified closed prelaunch configuration is `MY_TRUSTHUB_ENABLED=true`, `MY_TRUSTHUB_CANARY_ONLY=true`, `MY_TRUSTHUB_SIGNUP_ENABLED=false`, `MY_TRUSTHUB_EXPORT_ENABLED=true`, and `MY_TRUSTHUB_DELETE_ENABLED=true`; Supabase public signup remains disabled. Cloudflare Turnstile is enabled in Supabase Auth Attack Protection and the public site key is configured as `NEXT_PUBLIC_MY_TRUSTHUB_TURNSTILE_SITE_KEY`; the secret remains in Supabase. The founder canary remains intact and no synthetic user or contractor change was created.

Final backup evidence: physical backup `2026-09-10 05:28:13 UTC`, after P19 completion at `2026-09-09 14:28:44 UTC`; restore is available and Founder is the recovery operator.
