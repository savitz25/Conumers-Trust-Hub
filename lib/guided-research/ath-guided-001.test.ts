...
test('result-bearing clarification survives deterministic reload and resume',async()=>{
  const initial=await orchestrateGuidedResearch({action:{type:'START',question:'general contractor in New Jersey'}});
  assert.equal(initial.diagnostics.specialistCalls,1);assert.equal(initial.session.phase,'CLARIFY');assert.equal(initial.session.lastExecution?.source,'specialist');assert.equal(initial.session.lastExecution?.resultBearing,true);assert.equal(initial.session.lastExecution?.resultState,'UNSUPPORTED_TRADE_CAPABILITY');
  let stored=JSON.parse(JSON.stringify(initial.session));
  for(let cycle=0;cycle<20;cycle+=1){const resumed=await orchestrateGuidedResearch({session:stored,action:{type:'RESUME'}});assert.equal(resumed.diagnostics.specialistCalls,1);assert.equal(resumed.session.phase,'CLARIFY');assert.equal(resumed.result?.resultState,'UNSUPPORTED_TRADE_CAPABILITY');assert.equal(resumed.result?.error?.code,'no_new_jersey_statewide_general_contractor_class');assert.equal(resumed.result?.rows.length,0);assert.match(resumed.result?.consumerMessage??'',/does not have one unified statewide General Contractor credential class/i);assert.ok(resumed.session.availableChoices.some(choice=>choice.value==='contractor_trade:home_improvement'));stored=JSON.parse(JSON.stringify(resumed.session));}
  const ordinary=await orchestrateGuidedResearch({action:{type:'START',question:'I need a contractor'}});
  const ordinaryResume=await orchestrateGuidedResearch({session:JSON.parse(JSON.stringify(ordinary.session)),action:{type:'RESUME'}});
...
test('every specialist result-bearing clarification re-executes with its explanation and choices',async()=>{
  const cases=[
    ...
    ['general contractor in New Jersey','UNSUPPORTED_TRADE_CAPABILITY','no_new_jersey_statewide_general_contractor_class'],
...
test('New Jersey credential jurisdiction remains distinct from an out-of-state recorded address',async()=>{
  const hic=await orchestrateGuidedResearch({action:{type:'START',question:'home improvement contractor in New Jersey'}});
  assert.equal(hic.result?.rows[0].name,'#1 GARAGE DOOR LLC');assert.equal(hic.result?.rows[0].recordedLocation,'Orlando, Orange, FL');assert.ok(hic.result?.rows[0].facts.some(fact=>fact.label==='Credential jurisdiction'&&fact.value==='New Jersey'));
  assert.match(hic.result?.consumerMessage??'',/credential jurisdiction/i);assert.ok(hic.result?.limitations.some(value=>/recorded address.*service territory/i.test(value)));
});
