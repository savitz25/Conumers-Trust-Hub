...
export function hubLabel(hub: CustomerHubId): string {
  return hub === 'contractor' ? 'ContractorTrustHub' : hub === 'move' ? 'MoveTrustHub' : hub === 'lender' ? 'LenderTrustHub' : hub === 'investor' ? 'InvestorTrustHub' : hub === 'insurance' ? 'InsuranceTrustHub' : 'SeniorTrustHub';
}

export function identifierLabel(hub: CustomerHubId): string {
  return hub === 'move' ? 'USDOT' : hub === 'lender' ? 'Institution NMLS' : hub === 'senior' ? 'CMS CCN' : hub === 'investor' ? 'Firm CRD' : hub === 'insurance' ? 'NAIC Company Code' : 'Credential';
}
