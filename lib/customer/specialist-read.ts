...
export function compositeCustomerDirectory(contractor:import('./adapter.ts').CthDirectory):CustomerProfileDirectory{return{async getExact(p){if(p.hub_id==='contractor'){const row=await contractor.getById(p.native_profile_id,{sourceSystem:p.source_system,externalKey:p.external_key});return row?{...row,hubId:'contractor',publicationEligible:!row.isThin,entityClass:'contractor',canonicalUrl:`https://www.contractortrusthub.com/contractors/${row.slug}`}:null}return specialistCustomerDirectory.getExact(p)}}}
