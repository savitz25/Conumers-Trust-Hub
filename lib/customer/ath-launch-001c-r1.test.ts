...
test('existing staff session authorization is reused',()=>{assert.match(page,/readSessionToken/);assert.match(page,/assertStaff/);assert.match(route,/readSessionToken/);assert.match(block,/requireStaff/);assert.doesNotMatch(page+route,/Basic |bearer|password/i);});
