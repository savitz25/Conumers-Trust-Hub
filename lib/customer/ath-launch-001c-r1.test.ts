...
test('QA URLs are real canonical relative routes without fake object identifiers',()=>{for(const path of ['/manage','/claim/help?category=profile_claim','/claim/help?category=authority_problem'])assert.match(block,new RegExp(path.replace(/[?]/g,'\\?')));assert.doesNotMatch(block,/claim\/status|https?:\/\/|\/manage\/invitations\/accept/);});
test('synthetic invitation uses a safe existing route and only its corrected state is newly deliverable',()=>{assert.match(block,/type:'TEAM_INVITATION'.*stateVersion:'r2-v2'.*actionPath:'\/manage'/);assert.equal((block.match(/,stateVersion:'r1-v1'/g)||[]).length,4);assert.match(block,/stateVersion:item\.stateVersion/);});
