...
const migration=readFileSync('schema/migrations/011_ath_customer_lifecycle_email.sql','utf8');
const mail=readFileSync('lib/customer/mail.ts','utf8');

test('state-transition mail uses a durable unique ledger',()=>{assert.match(migration,/UNIQUE\(event_type,object_type,object_id,state_version\)/);assert.match(store,/ON CONFLICT\(event_type,object_type,object_id,state_version\) DO NOTHING/);});
...
test('delivery ledger stores no address, body, subject, token, or credential columns',()=>{const ddl=migration.replace(/COMMENT ON TABLE[\s\S]*/,'');assert.doesNotMatch(ddl,/recipient_email|\bbody\b|\bsubject\b|\btoken\b|\bcredential\b/i);assert.match(ddl,/recipient_user_id/);});
