...
test('six cards use traceable metric IDs and preserve entity grains', () => {
  ...
  assert.deepEqual(home.displayed_metric_ids.investor, []);
...
test('ten-state routing metadata preserves asymmetric specialist coverage', () => {
  assert.deepEqual(Object.keys(coverage.jurisdictions), ['US-FL', 'US-NJ', 'US-CA', 'US-TX', 'US-WA', 'US-AZ', 'US-CO', 'US-VA', 'US-NY', 'US-IL']);
  ...
  assert.equal(coverage.jurisdictions['US-CO'].askPath, '/colorado');
  ...
  assert.match(source, /Ten-state network explorer/);
});
