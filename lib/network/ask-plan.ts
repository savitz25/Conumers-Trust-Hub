...
export function buildNetworkAskPlan(query: string): NetworkAskPlan {
  ...
  if (parsed.geography?.stateCode === 'CA') {
    const caRoute = routeCaAsk(parsed.query);
    if (caRoute) {
      const already = hubs.some((h) => h.hubId === caRoute.hubId);
      if (!already) {
        hubs = [
          {
            hubId: caRoute.hubId,
            name: NETWORK_PUBLIC_NAMES[caRoute.hubId],
            capabilityStatus: 'handoff',
            destination: caRoute.destination,
            reason: caRoute.caveat,
            whatItCanAnswer: `California research on ${NETWORK_PUBLIC_NAMES[caRoute.hubId]}. Ask does not invent specialist facts.`,
            geographyCapability: parsed.geography?.meaning ?? 'California',
          },
          ...hubs,
        ];
      } else {
        hubs = hubs.map((h) =>
          h.hubId === caRoute.hubId
            ? {
                ...h,
                destination: caRoute.destination,
                geographyCapability: parsed.geography?.meaning ?? h.geographyCapability,
                reason: `${h.reason} ${caRoute.caveat}`,
              }
            : h,
        );
        hubs = [...hubs.filter((h) => h.hubId === caRoute.hubId), ...hubs.filter((h) => h.hubId !== caRoute.hubId)];
      }
    } else if (parsed.suggestedHubs[0]) {
      const primary = parsed.suggestedHubs[0];
      hubs = hubs.map((h) =>
  ...
  if (parsed.geography?.stateCode === 'OR') {
    const orRoute = routeOrAsk(parsed.query);
    const specificDestination = (dest?: string) =>
      Boolean(
        dest &&
          (/\/ask(\?|$)/i.test(dest) ||
    ...
    const annotateOr = (hub: NetworkAskHubPlan, caveat: string): NetworkAskHubPlan => {
      const keepDestination = hub.capabilityStatus === 'execute' || specificDestination(hub.destination);
      return {
        ...hub,
        destination: keepDestination ? hub.destination : orSpecialistUrl(hub.hubId),
        geographyCapability: hub.geographyCapability,
        reason: `${hub.reason} ${caveat}`,
        compareHref: keepDestination ? orSpecialistUrl(hub.hubId) : hub.compareHref,
      };
    };
    if (orRoute) {
      const already = hubs.some((h) => h.hubId === orRoute.hubId);
      if (!already) {
        hubs = [
          {
            hubId: orRoute.hubId,
            name: NETWORK_PUBLIC_NAMES[orRoute.hubId],
            capabilityStatus: 'handoff',
            destination: orRoute.destination,
            reason: orRoute.caveat,
            whatItCanAnswer: `Oregon research on ${NETWORK_PUBLIC_NAMES[orRoute.hubId]}. Ask does not invent specialist facts.`,
            geographyCapability: parsed.geography?.meaning ?? 'Oregon',
          },
          ...hubs,
        ];
      } else {
        hubs = hubs.map((h) => (h.hubId === orRoute.hubId ? annotateOr(h, orRoute.caveat) : h));
        hubs = [...hubs.filter((h) => h.hubId === orRoute.hubId), ...hubs.filter((h) => h.hubId !== orRoute.hubId)];
      }
    } else if (parsed.suggestedHubs[0]) {
      const primary = parsed.suggestedHubs[0];
      hubs = hubs.map((h) => (h.hubId === primary ? annotateOr(h, orCaveatForHub(primary)) : h));
    }
  }

  if (parsed.geography?.stateCode === 'PA') {
    const paRoute = routePaAsk(parsed.query);
    const specificDestination = (dest?: string) =>
      Boolean(
        dest &&
          (/\/ask(\?|$)/i.test(dest) ||
    ...
    const annotatePa = (hub: NetworkAskHubPlan, caveat: string): NetworkAskHubPlan => {
      const keepDestination = hub.capabilityStatus === 'execute' || specificDestination(hub.destination);
      return {
        ...hub,
        destination: keepDestination ? hub.destination : paSpecialistUrl(hub.hubId),
        geographyCapability: hub.geographyCapability,
        reason: `${hub.reason} ${caveat}`,
        compareHref: keepDestination ? paSpecialistUrl(hub.hubId) : hub.compareHref,
      };
    };
    if (paRoute) {
      const already = hubs.some((h) => h.hubId === paRoute.hubId);
      if (!already) {
        hubs = [
          {
            hubId: paRoute.hubId,
            name: NETWORK_PUBLIC_NAMES[paRoute.hubId],
            capabilityStatus: 'handoff',
            destination: paRoute.destination,
            reason: paRoute.caveat,
            whatItCanAnswer: `Pennsylvania research on ${NETWORK_PUBLIC_NAMES[paRoute.hubId]}. Ask does not invent specialist facts.`,
            geographyCapability: parsed.geography?.meaning ?? 'Pennsylvania',
          },
          ...hubs,
        ];
      } else {
        hubs = hubs.map((h) => (h.hubId === paRoute.hubId ? annotatePa(h, paRoute.caveat) : h));
        hubs = [...hubs.filter((h) => h.hubId === paRoute.hubId), ...hubs.filter((h) => h.hubId !== paRoute.hubId)];
      }
    } else if (parsed.suggestedHubs[0]) {
      const primary = parsed.suggestedHubs[0];
  ...
  if (parsed.geography?.stateCode === 'OH') {
    const ohRoute = routeOhAsk(parsed.query);
    const specificDestination = (dest?: string) =>
      Boolean(
        dest &&
          (/\/ask(\?|$)/i.test(dest) ||
    ...
    const annotateOh = (hub: NetworkAskHubPlan, caveat: string): NetworkAskHubPlan => {
      const keepDestination = hub.capabilityStatus === 'execute' || specificDestination(hub.destination);
      return {
        ...hub,
        destination: keepDestination ? hub.destination : ohSpecialistUrl(hub.hubId),
        geographyCapability: hub.geographyCapability,
        reason: `${hub.reason} ${caveat}`,
        compareHref: keepDestination ? ohSpecialistUrl(hub.hubId) : hub.compareHref,
      };
    };
    if (ohRoute) {
      const already = hubs.some((h) => h.hubId === ohRoute.hubId);
      if (!already) {
        hubs = [
          {
            hubId: ohRoute.hubId,
            name: NETWORK_PUBLIC_NAMES[ohRoute.hubId],
            capabilityStatus: 'handoff',
            destination: ohRoute.destination,
            reason: ohRoute.caveat,
            whatItCanAnswer: `Ohio research on ${NETWORK_PUBLIC_NAMES[ohRoute.hubId]}. Ask does not invent specialist facts.`,
            geographyCapability: parsed.geography?.meaning ?? 'Ohio',
          },
          ...hubs,
        ];
      } else {
        hubs = hubs.map((h) => (h.hubId === ohRoute.hubId ? annotateOh(h, ohRoute.caveat) : h));
        hubs = [...hubs.filter((h) => h.hubId === ohRoute.hubId), ...hubs.filter((h) => h.hubId !== ohRoute.hubId)];
      }
    } else if (parsed.suggestedHubs[0]) {
      const primary = parsed.suggestedHubs[0];
