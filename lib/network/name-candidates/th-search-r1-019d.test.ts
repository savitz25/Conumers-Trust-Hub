...
const pagination = (over: Partial<{ total: number; returned: number; hasMore: boolean; truncated: boolean; outOfRange: boolean }> = {}) =>
  ({ page: 1, limit: 10, returned: over.returned ?? 0, total: over.total ?? 0, reachable: over.total ?? 0, hasMore: over.hasMore ?? false, truncated: over.truncated ?? false, outOfRange: over.outOfRange ?? false, pageCount: 1, window: 200 });
...
const candidate = (over: CandOverrides = {}) => ({
  stableKey: over.stableKey ?? 'lender:nmls:401052', displayName: over.displayName ?? 'BMO Bank', entityType: over.entityType ?? 'Nonbank mortgage company',
...
test('02 BMO and Alliant candidate keys survive mapping; Randolph-Brooks FCU records stay separate, not merged', async () => {
  const bmo = await search('BMO Bank', 1, jsonFetch(() => ({ body: body({ candidates: [candidate({ stableKey: 'lender:nmls:401052', identifiers: [{ label: 'NMLS', value: '401052' }, { label: 'LEI', value: '3Y4U8VZURTYWI1W2K376' }] })], pag: pagination({ total: 1, returned: 1 }) }) })));
  ...
  const alliant = await search('Alliant Credit Union', 1, jsonFetch(() => ({ body: body({ candidates: [candidate({ stableKey: 'lender:nmls:197185', displayName: 'Alliant Credit Union', value: 'Alliant Credit Union', identifiers: [{ label: 'NMLS', value: '197185' }] })], pag: pagination({ total: 1, returned: 1 }) }, 'Alliant Credit Union') })));
  assert.equal(alliant.candidates[0].stableKey, 'lender:nmls:197185');
  const rb = await search('Randolph-Brooks FCU', 1, jsonFetch(() => ({ body: body({ resultState: 'AMBIGUOUS_EXACT_NAME', candidates: [
    candidate({ stableKey: 'lender:lei:AAAA', displayName: 'RANDOLPH-BROOKS', value: 'RANDOLPH-BROOKS', identifiers: [], method: 'ABBREVIATION_NORMALIZED', sourceLabel: 'HMDA reporter legal name' }),
    candidate({ stableKey: 'lender:nmls:583215', displayName: 'Randolph-Brooks Federal Credit Union', value: 'Randolph-Brooks Federal Credit Union', identifiers: [{ label: 'NMLS', value: '583215' }], method: 'ABBREVIATION_NORMALIZED' }),
  ...
  assert.deepEqual(rb.candidates.map((c) => c.stableKey), ['lender:lei:AAAA', 'lender:nmls:583215'], 'two distinct keys stay two distinct records');
...
test('04 punctuation/initialism, FCU, legal-suffix and historical/derived search forms are admitted, truthfully labeled', async () => {
  const vip = await search('VIP Mortgage Inc', 1, jsonFetch(() => ({ body: body({ candidates: [candidate({ stableKey: 'lender:hmda-lei:VIP1', displayName: 'V.I.P. MORTGAGE, INC.', value: 'V.I.P. MORTGAGE, INC.', publicationState: 'unpublished_research_identity', identifiers: [{ label: 'LEI', value: 'VIP1' }], action: null })], pag: pagination({ total: 1, returned: 1 }) }, 'VIP Mortgage Inc') })));
  ...
  const legalSuffix = await search('rocket mortgage llc', 1, jsonFetch(() => ({ body: body({ candidates: [candidate({ stableKey: 'lender:nmls:3030', displayName: 'Rocket Mortgage', value: 'Rocket Mortgage', method: 'LEGAL_SUFFIX_NORMALIZED', identifiers: [{ label: 'NMLS', value: '3030' }] })], pag: pagination({ total: 1, returned: 1 }) }, 'rocket mortgage llc') })));
  assert.equal(legalSuffix.candidates[0].matchMethod, 'NORMALIZED_NAME');
  const historical = await search('Old Dominion Mortgage', 1, jsonFetch(() => ({ body: body({ candidates: [candidate({ stableKey: 'lender:nmls:5001', displayName: 'Heritage Home Lending', method: 'DOCUMENTED_HISTORICAL_NAME', value: 'Old Dominion Mortgage', sourceLabel: 'historical name recorded on the published profile', identifiers: [{ label: 'NMLS', value: '5001' }] })], pag: pagination({ total: 1, returned: 1 }) }, 'Old Dominion Mortgage') })));
  ...
  const derived = await search('heritage home lending co', 1, jsonFetch(() => ({ body: body({ candidates: [candidate({ stableKey: 'lender:nmls:5001', displayName: 'Heritage Home Lending', method: 'DERIVED_SLUG_FORM', value: 'heritage home lending co', sourceLabel: "search form derived from the profile URL slug", identifiers: [{ label: 'NMLS', value: '5001' }] })], pag: pagination({ total: 1, returned: 1 }) }, 'heritage home lending co') })));
...
test('05 wrong echo/fingerprint, malformed rows and bad GLEIF identifiers are rejected -- never converted to misses', async () => {
  ...
  const categoryOnlyOverlap = await search('Rocket Mortgage', 1, jsonFetch(() => ({ body: body({ candidates: [candidate({ stableKey: 'lender:nmls:9', displayName: 'Generic Mortgage Company', value: 'Generic Mortgage Company' })], pag: pagination({ total: 1, returned: 1 }) }, 'Rocket Mortgage') })));
  ...
  const unrelatedNormalizedName = await search('Rocket Mortgage', 1, jsonFetch(() => ({ body: body({ candidates: [candidate({ stableKey: 'lender:nmls:9', displayName: 'Totally Unrelated Bank', value: 'Totally Unrelated Bank', method: 'EXACT_NORMALIZED_NAME' })], pag: pagination({ total: 1, returned: 1 }) }, 'Rocket Mortgage') })));
...
test('07 miss, restriction, source failure, partial/capped results and valid empty pages stay distinct; other hubs survive a Lender failure', async () => {
  ...
  const capped = await search('First', 1, jsonFetch(() => ({ body: body({ candidates: [candidate({ stableKey: 'lender:x:1', displayName: 'First National Bank', value: 'First National Bank' })], pag: pagination({ total: 200, returned: 1, hasMore: false, truncated: true }) }, 'First') })));
  ...
  const midWindow = await search('First', 1, jsonFetch(() => ({ body: body({ candidates: [candidate({ stableKey: 'lender:x:1', displayName: 'First National Bank', value: 'First National Bank' })], pag: pagination({ total: 200, returned: 10, hasMore: true, truncated: true }) }, 'First') })));
  ...
  const emptyPageInRange = await search('First', 4, jsonFetch(() => ({ body: body({ resultState: 'NO_MATCH', pag: pagination({ total: 74, returned: 0, hasMore: false, outOfRange: true }) }, 'First') })));
...
test('09 the supplied name is passed through verbatim across pages; a stale (superseded) response never overwrites newer state', async () => {
  const sent: string[] = [];
  await search('Rocket Mortgage, LLC', 2, jsonFetch((_url, init) => { const parsed = JSON.parse(String(init?.body)); sent.push(parsed.name); assert.equal(parsed.page, 2); return { body: body({ pag: pagination({ total: 0, returned: 0 }) }, 'Rocket Mortgage, LLC') }; }));
  ...
  const older = await search('BMO', 1, jsonFetch(() => ({ body: body({ candidates: [candidate({ stableKey: 'lender:old:1' })], pag: pagination({ total: 1, returned: 1 }) }, 'BMO') })));
  const newer = await search('BMO Bank', 1, jsonFetch(() => ({ body: body({ candidates: [candidate({ stableKey: 'lender:new:1' })], pag: pagination({ total: 1, returned: 1 }) }) })));
