...
type LenderCandidateOverrides = Partial<{ stableKey: string; displayName: string; entityType: string | null; method: string; value: string; sourceLabelText: string; explanation: string | null; identifiers: Array<{ label: string; value: string }>; publicationState: string; action: { type: string; url: string } | null; sourceAsOf: string | null }>;
const lenderCandidate = (over: LenderCandidateOverrides = {}) => ({
  stableKey: over.stableKey ?? 'lender:nmls:1001', displayName: over.displayName ?? 'First National Bank', entityType: over.entityType ?? 'Nonbank mortgage company',
  match: { method: over.method ?? 'EXACT_NORMALIZED_NAME', field: 'canonical_name', value: over.value ?? (over.displayName ?? 'First National Bank'), sourceLabel: over.sourceLabelText ?? 'published profile canonical name', explanation: over.explanation ?? 'exact match', isDocumentedSourceName: true },
...
test('R3b. ambiguity WITH records renders them as separate candidates (not merged); true miss, restricted-scope and a positive control are distinct', async () => {
  const rows = [lenderCandidate({ stableKey: 'lender:nmls:1001' }), lenderCandidate({ stableKey: 'lender:nmls:2002', identifiers: [{ label: 'NMLS', value: '2002' }], action: { type: 'PROFILE', url: 'https://www.lendertrusthub.com/lender/first-national-bank-2' } })];
  ...
  assert.equal(amb.state, 'COMPLETED_WITH_CANDIDATES'); assert.deepEqual(amb.candidates.map((c) => c.stableKey), ['lender:nmls:1001', 'lender:nmls:2002'], 'same-name institutions stay distinct -- multiple keys are not merged into one company');
...
test('R3d. a source-established alias is not discarded for having different words -- and its text is never invented', async () => {
  ...
  const historical = await lenderNameAdapter.search('Quicken Loans', 1, ctx(jsonFetch({ ...LENDER_V1, resultState: 'CANDIDATES', name: lenderName('Quicken Loans'), scope: {}, source: {}, candidates: [lenderCandidate({ displayName: 'Rocket Mortgage', method: 'DOCUMENTED_HISTORICAL_NAME', value: 'Quicken Loans', sourceLabelText: 'historical name recorded on the published profile', identifiers: [{ label: 'NMLS', value: '3030' }], action: { type: 'PROFILE', url: 'https://www.lendertrusthub.com/lender/rocket-mortgage' } })], pagination: lenderPagination(1), continuation: null, limitations: [] })));
