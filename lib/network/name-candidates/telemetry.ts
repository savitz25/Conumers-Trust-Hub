...
export type NameCandidateTelemetry = {
  ...
  reason: 'none' | 'partial_source_coverage' | 'completed_miss' | 'completed_miss_partial_scope' | 'partial_source_coverage_no_candidates' | 'ambiguous_upstream_no_candidates' | 'all_sources_failed';
  hubScope: string;
...
export function nameCandidateTelemetry(response: NameCandidateResponse): NameCandidateTelemetry {
  ...
  if (candidateCount > 0) return { ...base, terminalOutcome: 'RESULTS', reason: coverage.incompleteHubs.length || coverage.ambiguousHubs.length ? 'partial_source_coverage' : 'none' };
  ...
  if (coverage.ambiguousHubs.length) return { ...base, terminalOutcome: 'FAIL_CLOSED_WITH_ACTION', reason: 'ambiguous_upstream_no_candidates' };
