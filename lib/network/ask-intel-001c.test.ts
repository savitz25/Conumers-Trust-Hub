...
test('scope failures retain useful safe actions without pretending zero results',async()=>{for(const q of ['mover in Tampa Bay Florida','movers in Boca Raton Florida','registered investment advisers in West Palm Beach Florida','roofer in Phoenix Arizona']){const response=await orchestrateGuidedResearch({action:{type:'START',question:q}});assert.equal(response.diagnostics.specialistCalls,0,q);assert.notEqual(response.result?.resultState,'ZERO_MATCHING_ROWS',q);assert.ok(response.session.nextActions.length>0,q);if(/Tampa Bay/i.test(q))assert.ok(response.session.nextActions.every(a=>a.id!=='move.florida'));}});
