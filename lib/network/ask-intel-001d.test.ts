import test from 'node:test';import assert from 'node:assert/strict';import {readFileSync} from 'node:fs';import {buildAskResearchRoute} from './ask-research-route.ts';import {createGuidedSession} from '../guided-research/session.ts';import {safeConciergeUrl} from '../ai/safe-markdown.ts';
...
test('multi-hub journeys are ordered tickets using verified destinations',()=>{for(const [q,required,optional] of journeys){const r=buildAskResearchRoute(q);assert.ok(r.journey,q);assert.deepEqual(r.journey.steps.map(s=>s.hub),required,q);assert.deepEqual(r.journey.optionalSteps.map(s=>s.hub),optional,q);for(const step of [...r.journey.steps,...r.journey.optionalSteps])for(const d of step.destinations){const u=new URL(d.href);if(u.origin==='https://www.asktrusthub.com'){assert.equal(step.hub,'senior');assert.equal(u.pathname,'/ask');const next=buildAskResearchRoute(u.searchParams.get('q')!);assert.equal(next.plan.primaryHub,'senior');assert.equal(next.journey,undefined);}else assert.match(d.href,/^https:\/\/(?:www\.)?(?:move|lender|insurance|contractor|senior|investor)trusthub\.com/)}} });
...
test('interstate journey scope stops before the housing clause',()=>{const route=buildAskResearchRoute("I'm moving from New Jersey to Florida and buying a house.");assert.equal(route.requestedScope,'New Jersey to Florida');assert.deepEqual(route.journey?.steps.map(s=>s.hub),['move','lender','insurance'])});
...
test('Concierge aborts stale work, has a deadline, and keeps newer input usable',()=>{const source=readFileSync(new URL('../../components/ask-chat/ask-chat-panel.tsx',import.meta.url),'utf8');assert.match(source,/abortRef\.current\?\.abort\(\)/);assert.match(source,/55_000/);assert.match(source,/timed out/);assert.doesNotMatch(source,/disabled=\{loading \|\| !input\.trim\(\)\}/)});
