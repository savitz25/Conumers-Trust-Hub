...
import { AuthError, ManagementError, monitoringSourceSupported } from '@/lib/customer/store';
...
export default async function ManageProfilePage({ params }: { params: Promise<{ profileId: string }> }) {
  ...
  return <main className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:py-12">
    ...
    <header><Link href="/manage" className="text-sm text-muted-foreground underline">Back to My Trust Hub</Link>
      <p className="mt-5 text-xs font-semibold uppercase tracking-wider text-indigo">Business profile workspace</p>
    ...
    <section id="overview" className="card-surface p-5"><h2 className="text-xl font-semibold text-navy">What you manage</h2><p className="mt-2 text-sm text-muted-foreground">You manage business-supplied information, responses, correction requests, and available monitoring. Trust Hub and public-source evidence stays read-only and independent. Control verified, not endorsement.</p><dl className="mt-4 grid gap-3 text-sm sm:grid-cols-4"><div><dt className="text-muted-foreground">Owning Trust Hub</dt><dd>{capability.displayName}</dd></div><div><dt className="text-muted-foreground">Profile class</dt><dd>{customerEntityClassLabel(model.access.entity_class)}</dd></div><div><dt className="text-muted-foreground">Public identifier</dt><dd className="break-all">{capability.identifierLabel} {model.access.native_credential_key}</dd></div><div><dt className="text-muted-foreground">Your role</dt><dd>{model.access.role}</dd></div></dl></section>
    ...
    <div id="monitoring">{capability.monitoring==='SUPPORTED'&&monitoringSourceSupported(model.access.hub_id,String((monitoring.access as {native_source_system?:string}).native_source_system??''))?<MonitoringPanel profileId={profileId} canEdit={canEdit} initial={monitoring.subscription as never} notifications={monitoring.notifications as never}/>:<section className="card-surface p-5"><h2 className="font-semibold text-navy">Monitoring is not available yet</h2><p className="mt-2 text-sm text-muted-foreground">{capability.monitoring==='SUPPORTED'?'Exact-profile source monitoring currently covers Florida DBPR records only. It is not yet available for this credential source (for example New Jersey DCA registrations), and no regulatory alerts are generated for it.':`Exact-profile source monitoring is not yet available for ${capability.displayName}.`} You can still manage business information and review official evidence.</p><div className="mt-4 flex flex-wrap gap-3"><a className="btn-primary" href="#business-information">Keep managing this profile</a><a className="btn-secondary" href="#official-evidence">View official evidence</a><Link className="btn-secondary" href={`/claim/help?category=monitoring_unavailable&hub=${model.access.hub_id}&profile=${encodeURIComponent(profileId)}`}>Contact us about monitoring</Link></div></section>}</div>
