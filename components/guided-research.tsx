...
export function GuidedResearch({query}:{query:string}) {
  ...
  return <section className="space-y-6 pb-24 sm:pb-6" aria-busy={busy}>
    <div className="rounded-2xl border bg-white p-5 sm:p-6" style={{borderColor:ASK_BRAND.border,boxShadow:ASK_SHADOW.soft}}>
      ...
      {!busy&&session?.nextActions.some(action=>action.href||action.type==='CONSENT_BROADENING')?<section className="mt-5" aria-label="Next best actions"><h3 className="text-sm font-semibold" style={{color:ASK_BRAND.navy}}>Next best actions</h3><div className="mt-2 flex flex-wrap gap-2">{session.nextActions.filter(action=>action.href||action.type==='CONSENT_BROADENING').map(action=>action.href?<a key={action.id} href={action.href} className="inline-flex min-h-11 items-center rounded-xl border px-4 text-sm font-semibold" style={{borderColor:ASK_BRAND.border,color:ASK_BRAND.indigo}}>{action.label}</a>:<button key={action.id} type="button" onClick={()=>action.value&&void send({type:'SELECT_CHOICE',value:action.value})} className="min-h-11 rounded-xl border px-4 text-sm font-semibold" style={{borderColor:ASK_BRAND.border,color:ASK_BRAND.indigo}}>{action.label}</button>)}</div></section>:null}
...
function GuidedResults({result,session,onAction}:{result:GuidedExecutionResult;session:GuidedResearchSession;onAction:(action:GuidedAction)=>Promise<void>}){
  ...
  return <div className="space-y-6">
    ...
    {result.nextActions?.length?<section className="rounded-2xl border p-5" style={{borderColor:ASK_BRAND.border}}><h2 className="text-lg font-semibold" style={{color:ASK_BRAND.navy}}>Next best actions</h2><div className="mt-3 flex flex-wrap gap-2">{result.nextActions.filter(action=>action.href).map(action=><a key={action.id} href={action.href} className="inline-flex min-h-11 items-center rounded-xl border px-4 text-sm font-semibold" style={{borderColor:ASK_BRAND.border,color:ASK_BRAND.indigo}}>{action.label}</a>)}</div></section>:null}
