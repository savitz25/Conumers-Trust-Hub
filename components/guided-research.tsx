...
export function GuidedResearch({query}:{query:string}) {
  ...
  const send=useCallback(async(action:GuidedAction,current?:GuidedResearchSession|null)=>{
    ...
    try{
      ...
      sessionStorage.setItem(storageKey(query),JSON.stringify(body.session));
  ...
  return <section className="space-y-6" aria-busy={busy}>
    <div className="rounded-2xl border bg-white p-5 sm:p-6" style={{borderColor:ASK_BRAND.border,boxShadow:ASK_SHADOW.soft}}>
      ...
      {session?<p className="mt-2 text-sm" style={{color:ASK_BRAND.ink}}>Step: {session.phase.toLowerCase().replace('_',' ')} · Specialist: {session.hub==='senior'?'SeniorTrustHub':session.hub==='contractor'?'ContractorTrustHub':'MoveTrustHub'}</p>:null}
    ...
    {result?<GuidedResults result={result} onAction={send}/>:null}
  </section>;
}

function GuidedResults({result,onAction}:{result:GuidedExecutionResult;onAction:(action:GuidedAction)=>Promise<void>}){
  return <div className="space-y-6">
    ...
    {result.rows.length?<ol className="grid gap-4">{result.rows.map((row,index)=><li key={`${row.destination.href}-${index}`} className="rounded-2xl border bg-white p-5" style={{borderColor:ASK_BRAND.border,boxShadow:ASK_SHADOW.soft}}>
      ...
      <a href={row.destination.href} className="mt-4 inline-flex min-h-11 items-center font-semibold underline-offset-2 hover:underline" style={{color:ASK_BRAND.indigo}}>{row.destination.label}</a>
    </li>)}</ol>:null}
    {result.refinements.some(refinement=>refinement.values.length)?<section className="rounded-2xl border p-5" style={{borderColor:ASK_BRAND.border}}><h2 className="text-lg font-semibold" style={{color:ASK_BRAND.navy}}>What would you like to narrow or examine?</h2><div className="mt-3 space-y-4">{result.refinements.filter(r=>r.values.length).slice(0,3).map(refinement=><fieldset key={refinement.id}><legend className="text-sm font-semibold" style={{color:ASK_BRAND.navy}}>{refinement.label}</legend>{refinement.meaning?<p className="text-xs" style={{color:ASK_BRAND.ink}}>{refinement.meaning}</p>:null}<div className="mt-2 flex flex-wrap gap-2">{refinement.values.slice(0,8).map(item=><button key={item.value} type="button" onClick={()=>void onAction({type:'SET_FILTER',field:refinement.id,value:item.value})} className="min-h-11 rounded-full border px-3 text-sm" style={{borderColor:ASK_BRAND.border,color:ASK_BRAND.navy}}>{item.label}</button>)}</div></fieldset>)}</div></section>:null}
