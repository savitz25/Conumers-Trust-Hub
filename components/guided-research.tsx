...
export function GuidedResearch({query,initialSession,initialResult=null,routeDestinationHrefs=[]}:{query:string;initialSession:GuidedResearchSession;initialResult?:GuidedExecutionResult|null;routeDestinationHrefs?:string[]}) {
  ...
  const send=useCallback(async(action:GuidedAction,current?:GuidedResearchSession|null)=>{
    ...
    const started=performance.now();if(action.type==='EXECUTE'){trackEvent(ANALYTICS_EVENTS.ASK_SPECIALIST_EXECUTION_STARTED,{hub:(current??session)?.hub??'unknown'});captureTrustEvent(TRUSTHUB_EVENTS.SEARCH_SUBMITTED,{surface:'GUIDED',specialist_hub:(current??session)?.hub??undefined,capability:(current??session)?.researchPlan.intent,success:true});}
    setBusy(true);setResult(null);setError('');setResumeRecovery(false);
    try{
      const response=await fetch('/api/guided-research',{method:'POST',signal:controller.signal,headers:{'content-type':'application/json'},body:JSON.stringify({session:current??session,action})});
      const body=await response.json() as GuidedApiResponse&{message?:string};
      if(revision!==requestRef.current.revision)return;
      ...
      if(body.result){const common={hub:body.session.hub??'unknown',result_state:body.result.resultState,latency_bucket:bucketLatency(performance.now()-started),result_count_bucket:bucketResultCount(body.result.total),success:!['BACKEND_UNAVAILABLE','TIMEOUT'].includes(body.result.resultState)};if(body.result.executionOccurred!==false)trackEvent(ANALYTICS_EVENTS.ASK_SPECIALIST_RESULT_RECEIVED,common);trackEvent(ANALYTICS_EVENTS.SEARCH_TERMINAL_OUTCOME,{schema_version:'product_event.v1',hub:common.hub,intent:body.session.researchPlan.intent,terminal_outcome:guidedSearchTerminalOutcome(body.result.resultState,body.result.total,Boolean(body.result.destinations.length||body.result.nextActions?.length)),failure_reason:body.result.error?.code??(body.result.total?'none':body.result.resultState),next_action_type:body.result.nextActions?.[0]?.type??body.result.destinations[0]?.type??'none',result_count_bucket:common.result_count_bucket,duration_bucket:common.latency_bucket,route_family:'/ask',surface:'GUIDED'});captureTrustEvent(TRUSTHUB_EVENTS.SEARCH_RESULTS_RETURNED,{surface:'GUIDED',specialist_hub:body.session.hub??undefined,capability:body.session.researchPlan.intent,capability_state:body.result.resultState,result_count:body.result.total,success:common.success});}
  ...
  return <section className="space-y-6 pb-24 sm:pb-6" aria-busy={busy}>
    <div className="rounded-2xl border bg-white p-5 sm:p-6" style={{borderColor:ASK_BRAND.border,boxShadow:ASK_SHADOW.soft}}>
      ...
      {!busy&&session&&(session.phase==='COLLECT'||session.nextActions.some(action=>action.type==='ENTER_ENTITY_NAME'||action.type==='ENTER_IDENTIFIER'))?<form onSubmit={submit} className="mt-4">
        ...
        <div className="mt-2 flex flex-col gap-2 sm:flex-row"><input id="guided-value" value={value} onChange={event=>setValue(event.target.value)} className="min-h-12 min-w-0 flex-1 rounded-xl border px-4" data-ph-mask="true" style={{borderColor:ASK_BRAND.border,color:ASK_BRAND.navy}}/><button className="min-h-12 rounded-xl px-5 font-semibold text-white" style={{backgroundColor:ASK_BRAND.indigo}}>Continue</button></div>
        {session.missingFields.includes('geography')&&session.hub==='senior'?<p className="mt-2 text-xs" style={{color:ASK_BRAND.ink}}>Enter the state for the requested location, such as Texas. Choosing a state retains the city you already requested.</p>:null}
      </form>:null}
...
function consumerHandoffHref(href:string){return rewriteMoveSpecialistHref(href);}
function GuidedResults({result,session,onAction,excludedHrefs}:{result:GuidedExecutionResult;session:GuidedResearchSession;onAction:(action:GuidedAction)=>Promise<void>;excludedHrefs:string[]}){
  ...
  const excluded=new Set(excludedHrefs.map(consumerHandoffHref));
  const visibleNext=(result.nextActions??[]).filter(action=>action.href&&!excluded.has(consumerHandoffHref(action.href)));
  return <div className="space-y-6">
    ...
    {result.classPreviews?.some(group=>group.rows.length)?<div className="space-y-5">{result.classPreviews.map(group=>group.rows.length?<section key={group.providerClass} aria-label={`${group.label} preview`}>
      ...
      <ol className="mt-3 grid gap-3">{group.rows.map((row,index)=><li key={`${group.providerClass}-${row.identifier?.value??row.name}-${index}`} className="rounded-2xl border bg-white p-4" style={{borderColor:ASK_BRAND.border,boxShadow:ASK_SHADOW.soft}}>
        ...
        {row.destination?<a href={consumerHandoffHref(row.destination.href)} className="mt-3 inline-flex min-h-11 items-center font-semibold underline-offset-2 hover:underline" data-ath-event="search_result_opened" data-ath-hub={row.hub} data-ath-surface="GUIDED" style={{color:ASK_BRAND.indigo}}>{row.destination.label}</a>:null}
      </li>)}</ol>
    ...
    {result.rows.length?<ol className="grid gap-4">{result.rows.map((row,index)=><li key={`${row.destination?.href??row.identifier?.value??row.name}-${index}`} className="rounded-2xl border bg-white p-5" style={{borderColor:ASK_BRAND.border,boxShadow:ASK_SHADOW.soft}}>
      ...
      {row.destination?<a href={consumerHandoffHref(row.destination.href)} className="mt-4 inline-flex min-h-11 items-center font-semibold underline-offset-2 hover:underline" data-ath-event="search_result_opened" data-ath-hub={row.hub} data-ath-surface="GUIDED" style={{color:ASK_BRAND.indigo}}>{row.destination.label}</a>:<p className="mt-4 text-xs" style={{color:ASK_BRAND.ink}}>No public destination is available for this row. Verify with the source board where appropriate.</p>}
    </li>)}</ol>:null}
    ...
    {result.destinations.length&&!result.rows.length?<div className="flex flex-wrap gap-3">{result.destinations.map(destination=><a key={destination.href} href={consumerHandoffHref(destination.href)} className="inline-flex min-h-11 items-center rounded-xl border px-4 font-semibold" data-ath-event="specialist_handoff_started" data-ath-hub={session.hub??undefined} data-ath-surface="GUIDED" style={{borderColor:ASK_BRAND.border,color:ASK_BRAND.indigo}}>{destination.label}</a>)}</div>:null}
    {result.nextActions?.some(action=>action.type==='RETRY')?<button type="button" onClick={()=>void onAction({type:'EXECUTE'})} className="min-h-11 rounded-xl border px-4 font-semibold focus-visible:ring-2" style={{borderColor:ASK_BRAND.border,color:ASK_BRAND.indigo}}>Try again</button>:null}
    {!result.rows.length?<button type="button" onClick={()=>{const input=document.getElementById('ask-q') as HTMLInputElement|null;input?.focus();input?.select();}} className="min-h-11 rounded-xl border px-4 font-semibold focus-visible:ring-2" style={{borderColor:ASK_BRAND.border,color:ASK_BRAND.indigo}}>Edit question</button>:null}
    {visibleNext.length?<section className="rounded-2xl border p-5" style={{borderColor:ASK_BRAND.border}}><h2 className="text-lg font-semibold" style={{color:ASK_BRAND.navy}}>Next best actions</h2><div className="mt-3 flex flex-wrap gap-2">{visibleNext.map(action=><a key={action.id} href={consumerHandoffHref(action.href!)} className="inline-flex min-h-11 items-center rounded-xl border px-4 text-sm font-semibold" data-ath-event="specialist_handoff_started" data-ath-hub={session.hub??undefined} data-ath-surface="GUIDED" style={{borderColor:ASK_BRAND.border,color:ASK_BRAND.indigo}}>{action.label}</a>)}</div></section>:null}
    <details className="rounded-2xl border p-5" style={{borderColor:ASK_BRAND.border}}><summary className="flex min-h-11 cursor-pointer items-center font-semibold" style={{color:ASK_BRAND.navy}}>Why these results and where they came from</summary><dl className="mt-3 grid gap-2 text-sm sm:grid-cols-2">{result.interpretation.map(row=><Fact key={row.label} label={row.label} value={row.value}/>)}</dl>{result.dispatch?<p className="mt-3 break-words text-xs">Request sent: {result.dispatch.hub} / {result.dispatch.providerClass?.replaceAll('_',' ')} / <span data-ph-mask>{[result.dispatch.geography?.value,result.dispatch.geography?.state].filter(Boolean).join(', ')}</span> / {result.dispatch.endpoint}</p>:null}<ul className="mt-4 list-disc space-y-1 pl-5 text-sm" style={{color:ASK_BRAND.ink}}>{result.limitations.map(item=><li key={item}>{item}</li>)}</ul><p className="mt-3 text-xs" style={{color:ASK_BRAND.ink}}>Contract: {result.provenance.contract} · Specialist latency: {result.latencyMs} ms. {result.executionOccurred===false?'No provider retrieval ran; this is a capability decision.':'Specialist evidence is re-executed; stored client rows are never trusted.'}</p></details>
  </div>;
}
function Fact({label,value}:{label:string;value:string}){return <div><dt className="text-xs uppercase tracking-wide" style={{color:ASK_BRAND.ink}}>{label}</dt><dd className="font-medium" style={{color:ASK_BRAND.navy}} {...(isAskReplayMaskedEchoLabel(label)?{'data-ph-mask':true}:{})}>{value}</dd></div>}
