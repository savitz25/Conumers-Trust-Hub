  ...
  return <div className="space-y-6">
    ...
    {result.classPreviews?.some(group=>group.rows.length)?<div className="space-y-5">{result.classPreviews.map(group=>group.rows.length?<section key={group.providerClass} aria-label={`${group.label} preview`}>
      ...
      <ol className="mt-3 grid gap-3">{group.rows.map((row,index)=><li key={`${group.providerClass}-${row.identifier?.value??row.name}-${index}`} className="rounded-2xl border bg-white p-4" style={{borderColor:ASK_BRAND.border,boxShadow:ASK_SHADOW.soft}}>
        ...
        {row.destination?<a href={row.destination.href} className="mt-3 inline-flex min-h-11 items-center font-semibold underline-offset-2 hover:underline" data-ath-event="search_result_opened" data-ath-hub={row.hub} data-ath-surface="GUIDED" style={{color:ASK_BRAND.indigo}}>{row.destination.label}</a>:null}
      </li>)}</ol>
    ...
    <details className="rounded-2xl border p-5" style={{borderColor:ASK_BRAND.border}}><summary className="flex min-h-11 cursor-pointer items-center font-semibold" style={{color:ASK_BRAND.navy}}>Why these results and where they came from</summary><dl className="mt-3 grid gap-2 text-sm sm:grid-cols-2">{result.interpretation.map(row=><Fact key={row.label} label={row.label} value={row.value}/>)}</dl>{result.dispatch?<p className="mt-3 break-words text-xs">Request sent: {result.dispatch.hub} / {result.dispatch.providerClass?.replaceAll('_',' ')} / {[result.dispatch.geography?.value,result.dispatch.geography?.state].filter(Boolean).join(', ')} / {result.dispatch.endpoint}</p>:null}<ul className="mt-4 list-disc space-y-1 pl-5 text-sm" style={{color:ASK_BRAND.ink}}>{result.limitations.map(item=><li key={item}>{item}</li>)}</ul><p className="mt-3 text-xs" style={{color:ASK_BRAND.ink}}>Contract: {result.provenance.contract} · Specialist latency: {result.latencyMs} ms. {result.executionOccurred===false?'No provider retrieval ran; this is a capability decision.':'Specialist evidence is re-executed; stored client rows are never trusted.'}</p></details>
  </div>;
}
function Fact({label,value}:{label:string;value:string}){return <div><dt className="text-xs uppercase tracking-wide" style={{color:ASK_BRAND.ink}}>{label}</dt><dd className="font-medium" style={{color:ASK_BRAND.navy}} {...(label==='You asked'?{'data-ph-mask':true}:{})}>{value}</dd></div>}
