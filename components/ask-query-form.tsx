...
export function AskQueryForm({query}:{query:string}){
  ...
  return <form action="/ask" method="get" className="mb-10 max-w-2xl" role="search" aria-label="Ask the TrustHub Network" onSubmit={() => captureSearchSubmitted('ask_form')}>
    <label htmlFor="ask-q" className="sr-only">What do you want to know?</label>
    <div className="flex flex-col gap-2 sm:flex-row">
      <input id="ask-q" name="q" maxLength={ASK_QUESTION_MAX_LENGTH} value={value} onChange={event=>setValue(event.target.value)} placeholder="What do you want to know?" className="min-h-12 min-w-0 flex-1 rounded-xl border px-4" data-ph-mask="true" style={{borderColor:ASK_BRAND.border,color:ASK_BRAND.navy}}/>
